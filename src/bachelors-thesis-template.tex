...
%    baselinestretch=1.1,
    ...
    inputencoding=utf8,
    extendedchars=true,
    literate={õ}{{\~o}}1 {ö}{{\"o}}1 {ä}{{\"a}}1 {ü}{{\"u}}1,
}
...
%        \vspace*{1ex}
    ...
    Sigfox, LoRaWAN and NB\=/IoT are the best known LPWAN (Low Power Wide Area Network) technologies, which allow for large scale deployments of IoT applications.
    This survey assesses the suitability of those technologies to known IoT use cases by testing the available services in challenging environments near and within Tartu, Estonia.
    The resulting analysis gives insight about the best services to use for static sensor, mobile tracking or remotely operated device based applications.
    Testing will be conducted on the FiPy development board by Pycom.
    In addition to the services, a self\=/made LoRaWAN gateway is also built and tested in parallel.
...
%        \vspace*{1ex}
    ...
    %Asjade internet on omandamas üha mastaapsemaid mõõtmeid ning pidevalt on juurde tekkimas uusi ettepanekuid eri eluvaldkonna probleemide lahendamiseks nutikate seadmetega.
    %Traadita pisiseadmetele ühenduvuse tagamiseks pole otstarbekas kasutada seniseid laia tarbe sidetehnoloogiaid, vaid nende jaoks loodud väiksema jalajäljega LPWAN ehk madala voolutarbe ja laia ulatusega võrke.
    Sigfox, LoRaWAN ja NB\=/IoT on hetkel aktuaalseimad LPWANid ehk madala voolutarbe ja laia ulatusega raadiovõrgud, mis võimaldavad mastaapsete asjade interneti rakenduste juurutamist.
    %igaüks oma implementatsiooniga minimaalsest sideprotokollist.
    Käesolev töö analüüsib tehnoloogiate sobivust levinud IoT kasutusjuhtudele, katsetades selleks Tartus kättesaadavaid teenuseid reaalsetes oludes linnas ja lähialadel.
    Levi analüüsi tulemustes kajastuvad parimad teenused staatilise sensorvõrgu, liikuvate jälgimisseadmete või kaugjuhitavate aktuaatorite rakenduses kasutamiseks.
...
% END OF TODO PAGE 
    ...
    \section*{Sissejuhatus}\addcontentsline{toc}{section}{Sissejuhatus}
...
% TAUSTA LÜHIKIRJELDUS
    ...
    %Suurenev vajadus nutistul põhinevate äri- ja mugavusrakenduste järele põhjustab ühest küljest võidujooksu uute IoT-lahenduste leiutamisele,
...
%    LPWANid on mõeldud rakendustele, mis eeldavad nii madalat voolutarvet kui laia leviala.
...
%Tema simulatsioonide kohaselt omab LoRa modulatsioon väiksemat bitiveategurit võrreldes digitaal-sagedusmodulatsiooniga.
    ...
    \begin{table}[h]
        \caption{Üldtabel tehnoloogiate tehniliste parameetrite kohta.}
%        \makebox[\textwidth][c]{
        \resizebox{\columnwidth}{!}{
        \begin{tabular}{cccccc}
            & Sigfox~\cite{sigfoxtech} & LoRaWAN~\cite{lorawanspec} & NB-IoT$^1$ & LTE-M$^1$ & RPMA$^2$ \\
            \toprule
            \footnotesize{sagedusala} & \multicolumn{2}{c}{\SI{868}{\mega\hertz} (Euroopas)}  & \multicolumn{2}{c}{\footnotesize{\makecell{Telia -- LTE riba 20 \\ (\sim\SI{800}{\mega\hertz})}}} & \SI{2,4}{\giga\hertz} \\
            \midrule
            \footnotesize{ribalaius} & \SI{100}{\hertz} & \SI{125}{\kilo\hertz} & \SI{180}{\kilo\hertz} & \SI{1.08}{\mega\hertz} & \SI{1}{\mega\hertz} \\
            \midrule
            \footnotesize{andmekiirus} & $100$ bps & $0{,}3-27$ kbps & $20$ kbps & $1$ Mbps & $80$ kbps \\
            \midrule
            \footnotesize{\makecell{maksimaalne \\ kiirgusvõime}} & \multicolumn{2}{c}{\SI{25}{\milli\watt}$/$\SI{14}{\deci\belm}} & \multicolumn{2}{c}{\SI{200}{\milli\watt}$/$\SI{23}{\deci\belm}} & \makecell{\SI{100}{\milli\watt}$/$ \\ \SI{20}{\deci\belm}}  \\
            \midrule
            \footnotesize{\makecell{ressiiveri \\ sensitiivsus}} & \SI{-142}{\deci\belm} & \SI{-122}{\deci\belm} & \footnotesize{\makecell{\SI{+20}{\deci\bel} üle LTE}} & \footnotesize{\makecell{\SI{+12}{\deci\bel} üle LTE}} & \SI{-142}{\deci\belm}~\cite{raza} \\
            \midrule
            \footnotesize{\makecell{üleslink paketi \\ suurus}} & $12$ baiti & kuni $243$ baiti & \multicolumn{2}{c}{ei kehti} & $10$ kb \\
            \midrule
            \footnotesize{allalink} & \footnotesize{\makecell{ainult vastusena \\ üleslingile, $8$ baiti}} & jah & jah & jah & jah \\
            \midrule
            \footnotesize{päevane kvoot} & \footnotesize{\makecell{olenevalt teenuspaketist, \\ kuid mitte rohkem kui \\ 140 üleslink ja \\ 4 allalink}} & \footnotesize{\makecell{teenusepakkuja \\ poolt sätestatud}} & \multicolumn{2}{c}{ei kehti} & - \\
            \midrule
...
class NORAnetNode:
    ...
    %\def\tabularxcolumn#1{m{#1}}
    \begin{table}[h]
    \caption{Teoreetilise side kvaliteedi tingmärgid võrkude lõikes.}
    {\newcolumntype{Y}{>{\centering\arraybackslash}X}
        \begin{tabularx}{\textwidth}{ c | Y  Y  Y }
            \scriptsize{\diagbox{side\\kvaliteet}{võrk}} & \raisebox{-0\totalheight}{\includegraphics[scale=1.25]{figures/sigfox-full.pdf}} & \raisebox{-0\totalheight}{\includegraphics[scale=2]{figures/lorawan-full.pdf}} & \raisebox{-0\totalheight}{\includegraphics[scale=1.25]{figures/nbiot-full.pdf}} \\
            \hline
                \makecell{
                    \raisebox{-\totalheight}
...
%            \hline
                \makecell{\raisebox{-0\totalheight}{\includegraphics[scale=2.2]{figures/iks.pdf}}} &
...
% BAKALAUREUSETÖÖ KOKKUVÕTE
    %•Töö tulemused vastavalt sissejuhatuses püstitatud uurimisülesandele
    %•Kui sissejuhatus lõpeb töö hüpoteesidega, siis tuleb kokkuvõte liigendada vastavalt hüpoteeside arvule ja näidata, kas neid õnnestus tõestada või mitte
    %•Kui sisse juhatuses hüpoteese ei püstitatud, siis tuleb kokku võttes esitada süstematiseeritult kõik töö käigus selgunud järeldused, kaasaarvatud need, mis olid ära toodud peatükkide lõpus
    %•Kokkuvõtteseitoodaenamväljauutinformatsioonijaseal reeglinaeiviidatakirjandusele
    \newpage
